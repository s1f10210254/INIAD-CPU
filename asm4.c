...
int main()
{
    int L_REVERSENIBBLE, L_TOUPPERCASE, L_NEXT, L_END,
...
trap(4);                // Display string at L_S0
    SETH(R0,0x1234);    // R0 = 0x12340000
    OR3(R0,R0,0x5678);  // R0 = 0x12345678
    SUB(R1,R1);         // R1 = 0
    SUB(R2,R2);         // R2 = 0
    SUB(R3,R3);         // R3 = 0
trap(2);                // Display R0-R3
    BL(L_REVERSENIBBLE);// Save return address & branch to L_REVERSENIBBLE 
    aligncode();
trap(2);                // Display R0-R3
trap(6);                // Display new-line
    SETH(R0,0x89ab);    // R0 = 0x89ab0000
    OR3(R0,R0,0xcdef);  // R0 = 0x89abcdef
    SUB(R1,R1);         // R1 = 0
    SUB(R2,R2);         // R2 = 0
    SUB(R3,R3);         // R3 = 0
trap(2);                // Display R0-R3
...
trap(2);                // Display R0-R3
trap(6);                // Display new-line
    BRA(L_NEXT);        // Goto the next function call
L_S0 = addr;
    string("Call function 1: Reverse 32-bit nibbles");
    aligndata();
...
trap(4);                // Display string at L_S1
    ...
    LDI16(R1,L_S3);     // R1 = L_S3
    SUB(R2,R2);         // R2 = 0
    SUB(R3,R3);         // R3 = 0
trap(5);                // Display strings at L_S2 and at L_S3
    BL(L_TOUPPERCASE);  // Save return address & branch to L_TOUPPERCASE
    aligncode();
trap(5);                // Display strings at L_S2 and at L_S3
trap(6);                // Display new-line
...
// Ending
    aligncode();
L_END = addr;
    LDI16(R0,L_S6);     // R0 = L_S4
trap(4);                // Display string at L_S4
trap(0);                // Stop simulation
L_S6 = addr;
    string("End.");
    aligndata();

// Function 1: Reverse 32-bit nibbles
    aligncode();
L_REVERSENIBBLE = addr;
